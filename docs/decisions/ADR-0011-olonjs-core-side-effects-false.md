...
4. **Tenant rebuild verifies the win.** After publishing, `apps/olonjs.io`'s main visitor chunk is expected to drop by **≥ 100 KB raw / ≥ 30 KB gzipped**. The post-flag Lighthouse mobile run is the acceptance test: expected delta is **LCP −500 ms, TBT −100 ms, Lighthouse +5–8 points**, complementing the gain from ADR-0010.
...
- Tenant main visitor chunk shrinks by ≥ 100 KB raw / ≥ 30 KB gzipped (estimated from Lighthouse "unused JavaScript" coverage report).
- Mobile LCP improves by ~500 ms and TBT by ~100 ms, contributing 5–8 Lighthouse points toward the ADR-0008 mobile-90 target.
- Future tenants benefit automatically — the win is package-level, not tenant-level. No documentation is required for tenants to opt in.
- The package contract becomes stricter and more useful: any future contributor adding a side-effect file will surface the issue at audit time rather than silently ship dead-code retention.
...
- [ ] Extend `packages/core/scripts/check-runtime-decoupling.mjs` to also flag top-level CSS imports, top-level `addEventListener`, and top-level mutations in any file reachable from either entry. Non-blocking; current audit was manual.
- [ ] Add a one-line `sideEffects` reference to `docs/PUBLISHING.md` under the "Verifying the published artifacts" subsection.
...
- [ ] After the first tenant rebuild post-flag, capture before/after `dist/stats.html` snapshots and link them from this ADR's References section.
