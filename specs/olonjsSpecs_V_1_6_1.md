...
# OlonJS Architecture Specifications v1.6.1 

**Status:** Draft  
**Version:** 1.6.1 (Complete Draft - Core-Agnostic Theme Transport, Named Shell/Menu Binding Clarification, Full v1.6 Consolidation)  
**Target:** Senior Architects / AI Agents / Enterprise Governance

**Scope v1.6.1:** This edition is intended as a complete replacement specification, not as an addendum. It preserves the valid architecture of v1.6 while correcting the shell/menu binding ambiguity that could mislead agents or implementers. It keeps the full architecture surface (MTRP, JSP, TBP, CIP, ECIP, IDAC, TOCC, BSDS, ASC, JEB, JAP, Appendix A) and clarifies four architectural laws that were under-specified before: Core theme agnosticism, explicit theme flattening/publication, `menu.json` as the source of truth for menu structures, and direct shell data fields as binding fields for named `menu.json` collections.

**Primary v1.6.1 clarifications:**
...
- `menu.json` is the source of truth for menu structures and named menu/link collections
- `site.json` owns shell structure and shell instance declaration, but not materialized menu payloads bound from `menu.json`
